...
function resolveMaterialForPhase(
  ...
  // Add all possible identifiers from the product
...
const MachineOperatorView: React.FC = () => {
  ...
  const handleScanSuccess = async (decodedText: string) => {
    ...
    try {
      ...
      if (hasPhase2or30 && !phase2or30Done) {
        setMaterialInfo(resolveMaterialForPhase(data, materials));
      }
  ...
  /* === TIMER LOGIC === */
  const startFindMaterial = (phaseId: string) => {
  ...
  const startSetupForPhase = (phaseId: string) => {
  ...
  useEffect(() => {
    ...
    setMaterialInfo(
      hasRelevantPhase && !isDone ? resolveMaterialForPhase(sheet, materials) : null
    );
  }, [sheet, materials]);
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-4xl mx-auto">
        ...
        {materialInfo && (
          <div className="p-4 my-4 border rounded-md bg-indigo-50">
            ...
            <p>
              <strong>{t("common.material") || "Material"}:</strong>{" "}
              {materialInfo.materialCode}
            </p>
            <p>
              <strong>{t("common.quantity") || "Quantity"}:</strong>{" "}
              {materialInfo.currentQuantity} / {materialInfo.initialQuantity}
            </p>
            <p>
              <strong>{t("common.location") || "Location"}:</strong>{" "}
              {materialInfo.location
                ? `${materialInfo.location.area}, Pos ${materialInfo.location.position}`
                : t("common.na") || "N/A"}
            </p>
          </div>
        )}
