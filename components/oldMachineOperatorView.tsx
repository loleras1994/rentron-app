...
const MachineOperatorView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    try {
      // Fetch production sheet by QR code
      const data = await api.getProductionSheetByQr(decodedText);
      setSheet(data);

      // Check if the sheet contains phase 2 or 3
      ...
      if (hasPhase2or30 && !phase2or30Done) {
        // Fetch fresh materials from the backend
        const materials = await api.getMaterials(); // Force refetch materials from the server
        setMaterialInfo(resolveMaterialsForPhase(data, materials));  // Update material info
      }
